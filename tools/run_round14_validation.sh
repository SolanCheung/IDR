...
source "${idr_root}/tools/resolve_aegis_root.sh"
aegis_root="$(resolve_aegis_root "${idr_root}")"
round11_evidence="${idr_root}/docs/audit/reviews/2026-07-30-round11-independent"
round12_evidence="${idr_root}/docs/audit/reviews/2026-07-30-round12-independent"
round13_evidence="${idr_root}/docs/audit/reviews/2026-07-31-round13-independent"
...
printf '\n=== immutable Round 12 independent evidence ===\n'
(
    cd "${round12_evidence}"
    shasum -a 256 -c SHA256SUMS
)
printf 'ROUND12_EVIDENCE_ENTRIES=PASS\n'
printf '\n=== immutable Round 13 independent evidence ===\n'
(
    cd "${round13_evidence}"
    shasum -a 256 -c SHA256SUMS
)
printf 'ROUND13_EVIDENCE_ENTRIES=PASS\n'
printf 'AEGIS_ROOT_RESOLUTION=PASS (%s)\n' "$(basename "${aegis_root}")"
...
migration_database_name="$(
    psql "${migration_database_url}" -Atq -c "SELECT current_database()"
)"
migration_schema="idr_production_round14_validation_$$"
migration_runtime_role=""
migration_auditor_role=""
migration_owner_role=""
migration_old_runtime_role=""
migration_old_auditor_role=""
migration_login_role="idr_round14_runtime_login_$$"
migration_public_temp_was_granted="$(
    psql "${migration_database_url}" -Atq -c "
        SELECT CASE
            WHEN datacl IS NULL THEN true
            ELSE EXISTS (
                SELECT 1
...
migration_key_file="$(mktemp "${TMPDIR:-/tmp}/idr-round14-attestation-key.XXXXXX")"
...
cleanup_migration_fixture() {
    ...
    psql "${migration_database_url}" -v ON_ERROR_STOP=1 -q \
        -c "DROP ROLE IF EXISTS ${migration_login_role}" >/dev/null 2>&1 || true
    ...
    if [[ "${migration_public_temp_was_granted}" == "t" ]]; then
        psql "${migration_database_url}" -v ON_ERROR_STOP=1 -q \
            -c "GRANT TEMPORARY ON DATABASE ${migration_database_name} TO PUBLIC" \
            >/dev/null 2>&1 || true
    fi
...
psql "${migration_database_url}" -v ON_ERROR_STOP=1 -q -c "
    CREATE ROLE ${migration_login_role}
        LOGIN INHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE
        NOREPLICATION NOBYPASSRLS;
    GRANT ${migration_runtime_role} TO ${migration_login_role};
"
migration_boundary="$(
    psql "${migration_database_url}" -Atq -F '|' -c "
        ...
        SELECT
            ...
            NOT (SELECT rolinherit FROM pg_roles WHERE rolname = '${migration_runtime_role}'),
            (SELECT count(*) = 0
            ...
            has_function_privilege(
                '${migration_runtime_role}',
                '${migration_schema}.idr_open_attested_transition_v8(text,text,text,uuid,uuid,uuid,text,bigint,bigint,text,text,bigint,integer,uuid,bigint,text)',
                'EXECUTE'
            ),
            ...
            NOT has_database_privilege(
                '${migration_login_role}', current_database(), 'TEMPORARY'
            ),
            pg_has_role(
                '${migration_login_role}', '${migration_runtime_role}', 'MEMBER'
            ),
            pg_has_role(
                '${migration_login_role}', '${migration_runtime_role}', 'SET'
            ),
            pg_has_role(
                '${migration_login_role}', '${migration_runtime_role}', 'USAGE'
            ),
            (
                SELECT count(*) = 4
                   AND bool_and(
                       array_position(
                           procedure.proconfig,
                           'search_path=${migration_schema}, pg_catalog, pg_temp'
                       ) IS NOT NULL
                   )
            ...
            NOT EXISTS (
                SELECT 1
                 ...
                 WHERE candidate.rolname NOT IN (
                   ...
                   AND pg_has_role(
                       '${migration_login_role}', candidate.oid, 'SET'
                   )
            );
    " | tail -1
)"
if [[ "${migration_boundary}" != \
      "8|8|t|t|t|t|t|t|t|t|t|t|t|t|t|t|t" ]]; then
    printf 'Unexpected Production migration/role boundary: %s\n' "${migration_boundary}" >&2
    exit 1
fi
printf 'PRODUCTION_MIGRATOR_SCHEMA_VERSION=8\n'
...
printf 'PRODUCTION_RUNTIME_TEMPORARY=DENIED\n'
printf 'PRODUCTION_RUNTIME_ROLE_MEMBER_SET_USAGE=PASS\n'
printf 'PRODUCTION_RUNTIME_UNEXPECTED_SET_ROLES=0\n'
printf 'PRODUCTION_SECURITY_DEFINER_PATHS=4/4 PINNED\n'
printf 'PRODUCTION_RUNTIME_V8_ONLY=PASS\n'
...
printf '\n=== Round 14 targeted static checks ===\n'
for marker in \
...
for marker in \
    'idr_open_attested_transition_v8' \
    'idr_constant_time_equal_v8' \
    'SET search_path = %1$I, pg_catalog, pg_temp' \
    'REVOKE TEMPORARY ON DATABASE' \
    'ALTER ROLE %I NOINHERIT'; do
    rg -Fq "${marker}" \
        "${idr_root}/crates/idr-store/migrations/0008_round13_search_path_and_runtime_identity.sql"
done
for marker in \
    'can_create_temporary_objects' \
    'is_required_role_member' \
    'can_set_required_role' \
    'can_set_unexpected_role' \
    'security_definer_paths_are_pinned'; do
    rg -Fq "${marker}" "${idr_root}/crates/idr-runtime/src/postgres_authority.rs"
done
for attack in \
    ...
    'temporary_schema_shadow_cannot_replace_attestation_authority' \
    'legacy_opener_available'; do
    rg -Fq "${attack}" "${idr_root}/crates"
done
if rg -Fq 'SET search_path FROM CURRENT' \
    "${idr_root}/crates/idr-store/migrations/0008_round13_search_path_and_runtime_identity.sql"; then
    printf 'Round 13 migration inherited a SECURITY DEFINER search path.\n' >&2
    exit 1
fi
printf 'ROUND14_NEW_DEFINER_FROM_CURRENT=0\n'
for marker in \
    'IDR_AEGIS_ROOT' \
    '../Aegis-Life' \
    '../Aegis Life' \
    'crates/idr-aegis-adapter/Cargo.toml'; do
    rg -Fq "${marker}" "${idr_root}/tools/resolve_aegis_root.sh"
done
for script in \
    "${idr_root}/tools/run_round14_validation.sh" \
    "${idr_root}/tools/build_round14_audit_package.sh"; do
    rg -Fq 'source "${idr_root}/tools/resolve_aegis_root.sh"' "${script}"
    rg -Fq 'resolve_aegis_root "${idr_root}"' "${script}"
done
printf 'ROUND14_AEGIS_RESOLVER_STATIC_CHECK=PASS\n'
printf 'ROUND14_STATIC_CLOSURE_MARKERS=PASS\n'
...
printf 'API_KEY=%s%s\n' 'sk-' 'Round14SyntheticCredentialOnly000000' \
...
if [[ ${scanner_status} -eq 0 ]] ||
   ...
   grep -q 'Round14SyntheticCredentialOnly' <<<"${scanner_output}"; then
