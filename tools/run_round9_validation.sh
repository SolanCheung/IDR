...
run "immutable source evidence" shasum -a 256 \
    ...
    "${idr_root}/docs/audit/reviews/2026-07-30-round8-independent/IDR-V1.3-round8-independent-reaudit-report-2026-07-30.md" \
    "${idr_root}/docs/audit/reviews/2026-07-30-round8-independent/README-IDR-ROUND8-INDEPENDENT-REAUDIT.md" \
    "${idr_root}/docs/audit/reviews/2026-07-30-round8-independent/idr-v13-round8-independent-test-output.txt" \
    "${idr_root}/docs/audit/reviews/2026-07-30-round8-independent/idr-v13-round8-integrity-output.txt" \
    "${idr_root}/docs/audit/reviews/2026-07-30-round8-independent/idr-v13-round8-static-audit-checks.txt"
...
printf '\n=== Round 9 static authority checks ===\n'
...
if rg -n 'saturating_add|saturating_sub' \
    "${idr_root}/crates/idr-runtime/src/trust_chain.rs"; then
    printf 'Saturating authority revision/time arithmetic remains.\n' >&2
    exit 1
fi
for required_pattern in \
    ...
    if ! rg -q "${required_pattern}" \
        "${idr_root}/crates/idr-protocol/src/production.rs" \
        "${idr_root}/crates/idr-runtime/src/trust_chain.rs"; then
        printf 'Required signed identity/validity field is absent: %s\n' \
            "${required_pattern}" >&2
        exit 1
    fi
done
if ! rg -q 'reverify_bound_execution_authorization' \
    "${idr_root}/crates/idr-runtime/src/postgres_authority.rs" ||
   ! rg -q 'verify_authoritative_record_value_v1' \
    "${idr_root}/crates/idr-runtime/src/postgres_authority.rs" ||
   ! rg -q 'record_set_root' \
    "${idr_root}/crates/idr-runtime/src/postgres_authority.rs" ||
   ! rg -q 'idr_reject_authority_mutation_v4' \
    "${idr_root}/crates/idr-store/migrations/0004_round9_domain_validity_and_immutability.sql"; then
    printf 'Round 9 authorization/integrity closure marker is absent.\n' >&2
    exit 1
fi
printf 'SATURATING_AUTHORITY_ARITHMETIC=0\n'
printf 'SIGNED_TRUST_IDENTITY_FIELDS=PASS\n'
printf 'AUTHORIZATION_REVERIFY=PASS\n'
printf 'AUTHORITATIVE_DIGEST_REPLAY=PASS\n'
printf 'CHECKPOINT_RECORD_SET_ROOT=PASS\n'
printf 'APPEND_ONLY_DATABASE_GUARDS=PASS\n'
