...
output_input="${1:-/Users/solan/Downloads/IDR-V1.3-trust-chain-closure-round14-2026-07-31.zip}"
if [[ ! -d "$(dirname "${output_input}")" ]]; then
    printf 'Package output directory does not exist: %s\n' \
        "$(dirname "${output_input}")" >&2
    exit 2
fi
output_dir="$(cd "$(dirname "${output_input}")" && pwd -P)"
output_zip="${output_dir}/$(basename "${output_input}")"
...
stage_parent="$(mktemp -d "${TMPDIR:-/tmp}/idr-round14-package.XXXXXX")"
package_name="IDR-V1.3-trust-chain-closure-round14-2026-07-31"
...
copy_file \
    "${idr_root}/docs/audit/IDR_V1_3_ROUND14_PACKAGE_README.md" \
    "${package_root}/README.md"
copy_file \
    "${idr_root}/docs/audit/IDR_V1_3_ROUND14_AUDIT_MANIFEST.md" \
    "${package_root}/AUDIT-MANIFEST.md"
copy_file \
    "${idr_root}/docs/trust-chain-closure/ROUND14-REAUDIT-RESPONSE.md" \
    "${package_root}/ROUND14-REAUDIT-RESPONSE.md"
copy_file \
