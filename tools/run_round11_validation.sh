...
round10_evidence="${idr_root}/docs/audit/reviews/2026-07-30-round10-independent"
...
printf '\n=== immutable Round 10 independent evidence ===\n'
(
    cd "${round10_evidence}"
    shasum -a 256 -c SHA256SUMS
)
printf 'ROUND10_EVIDENCE_ENTRIES=PASS\n'
...
migration_schema="idr_production_round11_validation_$$"
...
cargo run --quiet \
    --manifest-path "${idr_root}/tools/idr-production-migrator/Cargo.toml" \
    --locked --offline -- "${migration_database_url}" "${migration_schema}"
migration_runtime_role="$(
    ...
    psql "${migration_database_url}" -Atq -F '|' -c "
        ...
        SELECT
            ...
            NOT has_column_privilege(
                '${migration_runtime_role}',
                '${migration_schema}.idr_execution_reservations',
                'idempotency_key',
                'UPDATE'
            ),
...
if [[ "${migration_boundary}" != "6|6|t|t|t|t|t|t" ]]; then
    printf 'Unexpected Production migration/role boundary: %s\n' "${migration_boundary}" >&2
    exit 1
fi
printf 'PRODUCTION_MIGRATOR_SCHEMA_VERSION=6\n'
printf 'PRODUCTION_RUNTIME_ROLE_BOUNDARY=PASS\n'
...
printf '\n=== Round 11 targeted static checks ===\n'
for marker in \
...
for marker in \
    'idr_operation_idempotency_fence_update_guard_v6' \
    'idr_execution_reservation_fence_guard_v6' \
    'idr_shadow_runtime_v1' \
    'idr_shadow_auditor_v1' \
    'UPDATE (state, aggregate_version, updated_at)'; do
    rg -Fq "${marker}" \
        "${idr_root}/crates/idr-store/migrations/0006_round11_runtime_roles_and_idempotency_fence.sql"
done
for attack in \
    ...
    'production_runtime_role_cannot_disable_execution_guards' \
    'pre_restart_duplicate_reservation_is_blocked_by_append_only_fence'; do
    rg -Fq "${attack}" "${idr_root}/crates"
done
printf 'ROUND11_STATIC_CLOSURE_MARKERS=PASS\n'
...
printf 'API_KEY=%s%s\n' 'sk-' 'Round11SyntheticCredentialOnly000000' \
...
if [[ ${scanner_status} -eq 0 ]] ||
   ...
   grep -q 'Round11SyntheticCredentialOnly' <<<"${scanner_output}"; then
...
printf '\nALL_ROUND11_VALIDATION_STEPS=PASS\n'
