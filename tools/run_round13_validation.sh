...
migration_key_file="$(mktemp "${TMPDIR:-/tmp}/idr-round13-attestation-key.XXXXXX")"
...
printf '\n=== Round 13 targeted static checks ===\n'
for marker in \
...
printf 'ROUND13_NEW_DEFINER_FROM_CURRENT=0\n'
printf 'ROUND13_STATIC_CLOSURE_MARKERS=PASS\n'
...
printf 'API_KEY=%s%s\n' 'sk-' 'Round13SyntheticCredentialOnly000000' \
...
if [[ ${scanner_status} -eq 0 ]] ||
   ...
   grep -q 'Round13SyntheticCredentialOnly' <<<"${scanner_output}"; then
...
printf '\nALL_ROUND13_VALIDATION_STEPS=PASS\n'
